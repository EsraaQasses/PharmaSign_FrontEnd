...
export default function PharmacistForgotPassword() {
  ...
  const handleConfirmReset = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await authApi.confirmPasswordReset(phone.trim(), "pharmacist", otp.trim(), password, confirmPassword);
      if (res.success) {
          ...
          <View className="flex-1">
            <View className="mb-8">
              ...
              <Text className="text-base text-gray-500 leading-relaxed text-right">
                {step === 0 && "أدخل رقم الجوال المرتبط بحسابك كصيدلي وسنرسل لك رمز التحقق عبر تيليغرام."}
              ...
              <>
                ...
                <TouchableOpacity 
                  className={`h-14 rounded-xl flex-row items-center justify-center shadow-lg shadow-primary/30 mt-4 gap-2 ${
                    !phone.trim() || isLoading ? "bg-primary/50" : "bg-primary"
                  }`}
            ...
            {step === 1 && (
              <>
                ...
                <TouchableOpacity 
                  className={`h-14 rounded-xl flex-row items-center justify-center shadow-lg shadow-primary/30 gap-2 ${
